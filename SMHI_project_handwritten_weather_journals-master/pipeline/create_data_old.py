...
def create_case(filename,poppler_path,path_label,choose_cases,size_case,compt_book,pages=[0,-1],LABELLING = True):
    ...
    for pagenumber in range(pages[0],pages[1]):
        ...
        if (table1_size == [6,9]) and (table2_size == [7,9]):#if we correcly get the cases
            ...
            table1_pos[2:,-1][:,1] -= 20
            if(len(choose_cases[0])==0):
                final_table=  [table2_pos[choose_cases[2],choose_cases[3]].reshape(-1,4)]

            elif(len(choose_cases[2])==0):
                final_table=  [table1_pos[choose_cases[0],choose_cases[1]].reshape(-1,4)]

            else:
                final_table=  [table1_pos[choose_cases[0],choose_cases[1]].reshape(-1,4),table2_pos[choose_cases[2],choose_cases[3]].reshape(-1,4)]
            ...
            for table in final_table:
                for position in table:
                    ...
                    key_name=encode(compt_book,pagenumber,compt3) + "_"
                    print(key_name)
                    im=number
                    ...
                    print(os.path.join(path_label,"temp.png"))
