...
def main(source, arch="flor", transform=False, cv2=False, image="", train=False,
	test=False, kaldi_assets=False, lm=False, N=2, norm_accentuation=False,
	norm_punctuation=False, epochs=1000, batch_size=16):
    """
    ...
    """
    nn = ".." # <--- originally
    nn = "Open Source Neural Network"
    raw_path = os.path.join(nn, "raw", source)
    source_path = os.path.join(nn, "data", f"{source}.hdf5")
    output_path = os.path.join(nn, "output", source, arch)
    ...
    if transform:
        ...
        dtgen = DataGenerator(source=source_path,
                              ...
                              predict=(not kaldi_assets) and test)
        ...
        if kaldi_assets:
            ...
            lm = LanguageModel(output_path, N)
